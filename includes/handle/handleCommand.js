    ...
    if (commandName.startsWith(PREFIX)) {
      if (!command) {
        ...
        if (checker.bestMatch.rating >= 0.5) {
          ...
          return api.sendMessage(`🍭 𝗔𝗿𝗲 𝘆𝗼𝘂 𝗳𝗶𝗻𝗱𝗶𝗻𝗴『${(checker.bestMatch.target)}』𝗼𝗿 𝗻𝗼𝘁? ⚡ 𝗧𝘆𝗽𝗲 -𝗵𝗲𝗹𝗽 𝘁𝗼 𝘀𝗲𝗲 𝗰𝗼𝗺𝗺𝗮𝗻𝗱𝘀 💫`, threadID, messageID); 
        }
      }
    }
  ...
  command = [...commands.values()].find(cmd => (
   ...
   cmd.config.usePrefix === false
    ...
    if (command && command.config && command.config.hasPermssion && command.config.hasPermssion > permssion) { 
      return api.sendMessage(`⚠️ |𝚈𝚘𝚞 𝚑𝚊𝚟𝚎 𝚗𝚘 𝚙𝚎𝚛𝚖𝚒𝚜𝚜𝚒𝚘𝚗 𝚝𝚘 𝚞𝚜𝚎 𝚝𝚑𝚒𝚜 𝚌𝚘𝚖𝚖𝚊𝚗𝚍▯${command.config.name}▯ `, event.threadID, event.messageID); 
    }
