...
const fs = global.nodemodule["fs-extra"];
  ...
  var callback = () =>   api.sendMessage({body:`${name} 𝗡𝗔𝘄 𝗧𝘂𝗺𝗮𝗿 𝗗𝗣<😘`, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
...
var name = res.data.result.name 
        var callback = () =>  api.sendMessage({body:`${name} Here Your Dp 😗`, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
        () => fs.unlinkSync(__dirname + "/cache/1.png"),event.messageID); 
...
var data = await api.getUserInfoV2(res_ID);
    ...
    var link = data.link
    var callback = () => api.sendMessage({body:`${name} Here is dp,  you want? `, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
        () => fs.unlinkSync(__dirname + "/cache/1.png"),event.messageID); 
    ...
    if (args.join().indexOf('@') !== -1) 
      ...
      var callback = () => 
api.sendMessage({body:`${name} Dp is here 🫠`, attachment: fs.createReadStream(__dirname + "/cache/1.png")}, event.threadID,
        () => fs.unlinkSync(__dirname + "/cache/1.png"),event.messageID); 
    return request(encodeURI(`https://graph.facebook.com/${uid}/picture?height=1500&width=1500&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`)).pipe(fs.createWriteStream(__dirname+'/cache/1.png')).on('close',
        () => callback()); 
