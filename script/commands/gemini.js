...
async function convertImageToCaption(imageURL, api, event, inputText) {
  try {
    ...
    const response = await axios.get(`https://hiroshi-api-hub.replit.app/ai/gemini?ask=${encodeURIComponent(inputText)}&image_url=${encodeURIComponent(imageURL)}`);
    const caption = response.data.response;

    if (caption) {
      const formattedCaption = formatFont(caption);
      api.sendMessage(`🚀 𝙂𝙚𝙢𝙞𝙣𝙞 𝘼𝙞 - 𝘼𝙣𝙖
...
module.exports.handleEvent = async function ({ api, event }) {
  if (!(event.body.indexOf("Gemini") === 0 || event.body.indexOf("জেমিনি") === 0)) return;
  ...
  try {
    const response = await axios.get(`https://hiroshi-api-hub.replit.app/ai/gemini?ask=${encodeURIComponent(inputText)}`);
