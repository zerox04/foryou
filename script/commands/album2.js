...
module.exports.run = async function ({ api, event, args}) {
  ...
  const message = "🎬 𝗖𝗵𝗼𝗼𝘀𝗲 𝗮𝗻 𝗼𝗽𝘁𝗶𝗼𝗻𝘀 𝗕𝗮𝗯𝘆 🌥️\n"+"✿━━━━━━━━━━━━━━━━━━━━━━━✿\n"+ albumOptions.map((option, index) => `💨${index + 1}. ${option} 🍓`).join("\n")+"\n𝗧𝘆𝗽𝗲 𝗔𝗹𝗯𝘂𝗺 2 𝘀𝗲𝗲 𝗺𝗼𝗿𝗲 🫧\n✿━━━━━━━━━━━━━━━━━━━━━━━✿";
...
const validCommands = ['cartoon', 'photo', 'lofi', 'sad', 'islamic','funny','horny','anime','love','baby','lyrics','sigma','photo','aesthetic','cat','flower','ff','sex','girl','football'];
  ...
  if (args[0] === 'list'){
 try {
   const lRes = await axios.get(`${global.config.API}/dipto/album?list=dipto`);
...
const d1 = args[1]?args[1].toLowerCase() : '' ;
 if (!d1 || !validCommands.includes(d1))return;
    ...
    try {
        const response = await axios.get(`${global.config.API}/dipto/imgur?url=${encodeURIComponent(URL)}`);
  ...
  query2 = query;} else {
        ...
        const svRes = await axios.get(`${global.config.API}/dipto/album?add=${query2}&url=${imgurLink}`);
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
 ...
 api.unsendMessage(handleReply.messageID);
  ...
  try {
    const res = await axios.get(`${global.config.API}/dipto/album?type=${query}`);
