...
async function handleNyxImageCommand(api, event) {
  ...
  api.sendMessage("💫 |  𝙱𝚋𝚢 𝙰𝙸 𝙶𝚎𝚗𝚎𝚛𝚊𝚝𝚒𝚗𝚐 𝚙𝚛𝚘𝚖𝚙𝚝, 𝙿𝚕𝚎𝚊𝚜𝚎 𝚠𝚊𝚒𝚝...", event.threadID, event.messageID);

  try {
    const enctxt = encodeURI(txt);
    const url = `https://hazeyy-merge-apis-b924b22feb7b.herokuapp.com/api/girlfriend?prompt=${enctxt}&model=${txt2}`;
    ...
    responses.forEach((data, index) => {
      const path = __dirname + `/tmp/image${index + 1}.png`;
      fs.writeFileSync(path, Buffer.from(data, "binary"));
      paths.push(path);
    });

    const senderName = "🌻 𝐁𝐛𝐲 ( 𝙰𝙸 )";
    const message = `${senderName}\n\n𝙷𝚎𝚛𝚎'𝚜 𝚢𝚘𝚞𝚛 𝙸𝚖𝚊𝚐𝚎 𝚙𝚛𝚘𝚖𝚙𝚝 🖼️`;
...
async function convertVoiceToText(audioUrl, api, event) {
  try {
    api.sendMessage("💽 |  𝙱𝚋𝚢 𝙰𝙸 𝙲𝚘𝚗𝚟𝚎𝚛𝚝𝚒𝚗𝚐 𝙰𝚞𝚍𝚒𝚘, 𝙿𝚕𝚎𝚊𝚜𝚎 𝚠𝚊𝚒𝚝...", event.threadID);
...
module.exports.handleEvent = async function ({ api, event, Users }) {
  if (!(event.body.toLowerCase().startsWith("bby") || event.body.toLowerCase().startsWith("Bby"))) return;
  ...
  try {
    api.sendMessage("💭 | 𝙱𝚋𝚢 𝙰𝙸 𝚒𝚜 𝚝𝚑𝚒𝚗𝚔𝚒𝚗𝚐...", event.threadID, event.messageID);

    let senderName = (await Users.getData(event.senderID)).name;
    let { data } = await axios.post("https://hazeyy-merge-apis-b924b22feb7b.herokuapp.com/api/girlfriend", { messages: history[event.senderID], sender_name: senderName });

    if (data && data.content) {
      history[event.senderID].push(data);

      const formattedResponse = isFontEnabled ? `😻💋 𝐁𝐛𝐲 ( 𝐀𝐈 )\n\n❓ 𝐀𝐬𝐤: '${text}'\n\n${formatFont(data.content)}` : `😻💋 𝐁𝐛𝐲 ( 𝐀𝐈 )\n\n❓ 𝐀𝐬𝐤: '${text}'\n\n${data.content}`;
